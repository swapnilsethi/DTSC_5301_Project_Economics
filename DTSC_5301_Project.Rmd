...
```{r add_stimulus_checks}

df_weekly <- df_weekly %>% mutate(first_check = (if (date <= ymd("2020-04-15")) 0 else 1), second_check = (if (date <= ymd("2021-01-04")) 0 else 1), third_check = (if (date <= ymd("2021-03-17")) 0 else 1))

```


```{r}
lm <- lm(spend_all ~ 0 + gps_retail_and_recreation + emp + first_check + second_check + third_check, df_weekly)
summary(lm)
```
