...
df_weekly <- df_weekly %>% mutate(lagged_spend_one = spend_all - lag(spend_all, order_by = date), 
                                  lagged_spend_two = spend_all - lag(spend_all, n = 2, order_by = date),
                                  lagged_spend_three = spend_all - lag(spend_all, n = 3, order_by = date))
...
#df_weekly <- df_weekly %>% mutate(first_check = (if (date < ymd("2020-04-15") | date >= ymd("2020-07-15")) 0 
#                                                 else 1), 
#                                  second_check = (if (date < ymd("2021-01-04") | date >= ymd("2021-03-16")) 0 
#                                                  else 1), 
#                                  third_check = (if (date < ymd("2021-03-18") | date >= ymd("2021-06-18")) 0 
#                                                 else 1))
...
df_weekly <- df_weekly %>% filter(!is.infinite(third_check))
sum(sapply(df_weekly, is.infinite))
```
