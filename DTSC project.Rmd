...
## Read in Data from Opportunity Insights GitHub Repository
...
### Clean the Data for the Second Linear Model

Here we repeat the cleaning steps that we did for the first linear model, except now we're doing it on weekly data instead of daily data.
...
### Summarize Second Regression Experiment

Because many states stopped giving out pandemic unemployment checks early, our data is super helpful for comparing states that continued with pandemic benefits vs those states which cut benefits off early.  Below is a list of states that cut off benefits early.
...
### Dealing with missing values for Second Regression
...
In the next four code chunks, we prepare our national level data for visualizations.
...
# Plotting national spending over time
...
# Plotting national spending over time (split by income)
...
# Plotting national unemployment rate over time (split by income)
